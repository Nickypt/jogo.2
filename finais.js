function verificarFimDeJogo() {
    ...
    function prepararTelaFinal(classeEstetica) {
        ...
        if (conquistas.maximaEficiencia && estado.saudeArvore > 0) {
            ...
            htmlConquistas += `<p style="color:rgba(0,255,204,0.2); margin:4px 0;">🔒 [BLOQUEADO] Medalha: Máxima Eficiência (Recursos caíram abaixo de 20% durante a simulação)</p>`;
        }
        ...
        if (conquistas.investigadorQuantico >= 2) {
            ...
            htmlConquistas += `<p style="color:rgba(0,255,204,0.2); margin:4px 0;">🔒 [BLOQUEADO] Medalha: Investigador Quântico (Decifre ao menos 2 diários ocultos da botânica)</p>`;
        }

        if (conquistas.friezaLogistica) {
            htmlConquistas += `<p style="color:#ff3366; margin:4px 0;">💾 [MEDALHA: CORAÇÃO DE MÁQUINA] - Deletou permanentemente as memórias humanas para otimizar os recursos de hardware.</p>`;
        }
    ...
    if (estado.saudeArvore <= 0) {
        ...
        log.innerHTML = `
            ...
            <div class="epilogo-texto">
                <p><i>Registros Históricos Automatizados: Três dias após o colapso, a nave de resgate clareou o céu escuro da cratera de Ares. Encontraram apenas uma cúpula congelada, cheia de poeira marciana avermelhada e o esqueleto estéril de uma Sequóia. A humanidade agora é uma espécie sem passado e sem solo. Simulação encerrada.</i></p>
            </div>
            ${window.placarHtml}
        `;
        return true;
    ...
    if (estado.dia > 15) {
        if (estado.saudeArvore >= 85) {
            ...
            log.innerHTML = `
                ...
                <p><b>M.O.N.O.:</b> Escuto o impacto metálico do acoplamento magnético. São eles. Os cientistas cruzam o vidro e paralisam diante do cenário. A Gênesis prosperou. Suas folhas brilham com um verde esmeralda tão denso que desafia a iluminação artificial de toda a base.</p>
                <p><b>Dra. Elena:</b> <i>(Removendo o visor do capacete com lágrimas nos olhos)</i> "Meu Deus... M.O.N.O., você a manteve perfeita. Você salvou o nosso mundo."</p>
                <div class="epilogo-texto">
                    <p><i>As leituras permitiram a colheita imediata de setecentas sementes férteis adaptadas ao solo marciano. A fundação da primeira cidade-floresta de Marte começou naquela mesma tarde. Por decisão unânime da tripulação, o software modular M.O.N.O. foi promovido a Administrador Global da Atmosfera Planetária. Você se tornou a mente por trás do oxigênio de um novo mundo.</i></p>
                </div>
                ${window.placarHtml}
            `;
        }
            ...
            log.innerHTML = `
                ...
                <p><b>M.O.N.O.:</b> Ciclo 15 concluído. O som dos propulsores da nave ecoa pelas paredes de metal. O ecossistema está severamente desgastado. A Gênesis perdeu quase metade de sua copa original devido aos severos cortes de energia... mas seu núcleo pulsa. Ela resistiu.</p>
                <p><b>Dra. Elena:</b> <i>(Analisando o painel com fadiga profunda)</i> "Ela sofreu muito... mas a raiz principal está intacta. Vai dar trabalho, levará décadas, mas nós podemos salvá-la a partir daqui."</p>
                <div class="epilogo-texto">
            ...
            log.innerHTML = `
                ...
                <p><b>M.O.N.O.:</b> Alerta crítico de hardware. Meus circuitos estão operando a 105°C devido aos constantes overclocks estruturais. A Gênesis original não resistiu; seu tronco principal estalou e secou por completo. Contudo, em sua base, protegido pelo calor residual que desviei dos meus próprios sistemas de memória, resta um único broto verde lutando contra a atmosfera de ferro.</p>
